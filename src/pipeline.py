...
# System prompt rico para tasks executadas pelo Pipeline. Esta string e
# CONSTANTE entre tasks de uma mesma sessao — por isso o LLMClient
# (_call_anthropic) detecta o limiar de 4000 chars, envolve em
...
def _build_task_system_prompt(task: "Task") -> str:
    """Compoe o system prompt cacheable + sufixo dinamico curto.

    O bloco PIPELINE_SYSTEM_BASE (>4000 chars) atinge o limiar de cache
    do _call_anthropic e e enviado com cache_control: ephemeral. O sufixo
    dinamico (~80 chars) entra logo depois mas NAO quebra o cache porque
    a Anthropic faz cache pelo prefixo cumulativo — o trecho cacheavel
    fica intacto e so o sufixo paga preco normal.

    Resultado: tasks subsequentes na mesma sessao (5min TTL) pagam apenas
    10% dos tokens da base + tokens do sufixo + tokens user.
    """
    suffix = (
        f"\n\nTAREFA ATUAL\n"
        f"Voce esta executando a tarefa '{task.id}' (tipo: {task.type}). "
        f"Responda de forma precisa e direta, sem reformatacoes extras."
    )
    return PIPELINE_SYSTEM_BASE + suffix
...
class Pipeline:
    ...
    # Mapeamento conservador de max_tokens por task_type. A maioria das
    # tarefas nao precisa dos 8192 tokens default do LLMConfig — o teto
    # estava saturando em ~96% das calls de architecture historicamente,
    # gerando outputs verbosos sem ganho de qualidade. Reducao para 4096
    # forca saidas mais focadas.
    #
    ...
    def _max_tokens_for_task(cls, task_type: str, config: "LLMConfig") -> int:
        """Resolve max_tokens respeitando o limite do modelo.

        Antes da Sprint 8 (2026-04-08), o pipeline sempre passava
        config.max_tokens (8192 para Claude), mesmo para tasks que nao
        precisavam disso. Agora aplica um teto por tipo + cap pelo modelo.
        """
        wanted = cls._MAX_TOKENS_BY_TASK_TYPE.get(task_type, cls._DEFAULT_MAX_TOKENS)
        return min(wanted, config.max_tokens)
    ...
    async def _call_llm(
        ...
        try:
            response = await client.query(
                ...
                max_tokens=self._max_tokens_for_task(task.type, config),
            )
